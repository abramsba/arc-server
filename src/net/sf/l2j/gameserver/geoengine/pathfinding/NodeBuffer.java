...
public class NodeBuffer
{
	 ...
	 * @return NodeRef : first node of path
	 */
	public final Node findPath(int gox, int goy, short goz, int gtx, int gty, short gtz)
	{
		...
		while (_current != null && ++count < GameServer.Cfg.Geo.MaxIterations);
	...
	/**
	 * Check _current NodeRef and add its neighbors to the buffer.
	 */
	private final void expand()
	{
		...
		if ((nswe & GeoStructure.CELL_FLAG_N) != 0)
			addNode(x, y - 1, z, GameServer.Cfg.Geo.BaseWeight);
		...
		if ((nswe & GeoStructure.CELL_FLAG_W) != 0)
			addNode(x - 1, y, z, GameServer.Cfg.Geo.BaseWeight);
		...
		if ((nswe & GeoStructure.CELL_FLAG_NW) != 0)
			addNode(x - 1, y - 1, z, GameServer.Cfg.Geo.DiagonalWeight);
	 ...
	 * @return NodeRef : node, if exits in buffer
	 */
	...
	private final void addNode(int x, int y, short z, int weight)
	{
		...
		if (node.getLoc().getNSWE() != (byte) 0xFF)
			node.setCost(getCostH(x, y, node.getLoc().getZ()) + weight * GameServer.Cfg.Geo.ObstacleMultiplier);
		else
			node.setCost(getCostH(x, y, node.getLoc().getZ()) + weight);
		
		Node current = _current;
		int count = 0;
		while (current.getChild() != null && count < GameServer.Cfg.Geo.MaxIterations * 4)
		{
			count++;
		...
		if (count >= GameServer.Cfg.Geo.MaxIterations * 4)
	...
	private final double getCostH(int x, int y, int i)
	{
		...
		return Math.sqrt(dX * dX + dY * dY + dZ * dZ) * GameServer.Cfg.Geo.HeuristicWeight; // Direct distance
	}
}